...
final class ObjectTests: XCTestCase {
    ...
    func discoverRadio(logState: Api.NSLogging = .normal) -> Radio? {
        ...
        if discovery.radios.count > 0 {
            ...
            _ = Api.sharedInstance.connect(discovery.radios[0].packet, program: "ObjectTests", isGui: connectAsGui, logState: logState)
            sleep(2)
    ...
    func testAmplifierParse() {
        ...
        // since properties are updated on the main thread, wait for the updates to occur
    ...
    func testPanadapter() {
        ...
        DispatchQueue.main.async { [self] in
            if radio!.panadapters.count > 0 {
                existingObjects = true
                if showInfoMessages { Swift.print("\n***** \(radio!.panadapters.count) Existing \(type) object(s) will be removed") }
        ...
        if radio!.panadapters.count == 0 {
            ...
            if radio!.waterfalls.count == 1 {
                if let object = radio!.waterfalls.first?.value {
                    ...
                    if showInfoMessages { Swift.print("\n***** Remove 1st \(type): pan id = \(radio!.waterfalls[firstId]!.panadapterId?.hex ?? "0x0")") }
                    ...
                    if radio!.panadapters.count == 0 {
                        ...
                        if radio!.waterfalls.count == 1 {
                            ...
                            if let object = radio!.waterfalls.first?.value {
                                ...
                                object.autoBlackEnabled = false
                                object.blackLevel = 10
                                object.colorGain = 20
                                object.gradientIndex = 2
                                object.lineDuration = 50

                                if showInfoMessages { Swift.print("***** 2nd \(type) parameters modified") }

                                XCTAssertEqual(object.autoBlackEnabled, false, "AutoBlackEnabled", file: #function)
                                XCTAssertEqual(object.blackLevel, 10, "BlackLevel", file: #function)
                                XCTAssertEqual(object.colorGain, 20, "ColorGain", file: #function)
                                XCTAssertEqual(object.gradientIndex, 2, "GradientIndex", file: #function)
                                XCTAssertEqual(object.lineDuration, 50, "LineDuration", file: #function)
                                XCTAssertEqual(object.panadapterId, panadapterId, "Panadapter Id", file: #function)
